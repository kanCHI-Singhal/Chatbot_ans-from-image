...
def describe_image(image_content):
    """Describe an image using Qwen2.5-VL through HuggingFace Router API."""
    import base64
    import requests
    import os
    ...
    if not HF_API_KEY:
        return "ERROR: Set HF_API_KEY in your .env"
    ...
    try:
        data = response.json()
        print("DEBUG HF Response:", data)

        if "choices" in data:
            return data["choices"][0]["message"]["content"]

        if "error" in data:
            return "HF Router Error: " + str(data["error"])

        return "No valid response from Qwen2.5-VL."
    except Exception as e:
        return "HF Parsing Error: " + str(e)
        ...
        "messages": [
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": "Generate the underlying data table for the figure below."
                    },
                    {
                        "type": "image_url",
                        "image_url": {
